...
@section('content')
    <div class="m-6 w-full">
        <h1 class="w-full">Create Post</h1>

        <div class="w-3/6 m-5">
            {{ html()->form($method = 'POST', $action = '/store')->class('flex flex-col mx-auto max-w-l')->open() }}

            {{ html()->label($contents = 'Title', $for = 'title') }}
            {{ html()->text($name = 'title', $value = '')->placeholder($placeholder = 'title')->class('text-black p-3') }}

            {{ html()->label($contents = 'Body', $for = 'body') }}
            {{ html()->textarea($name = 'body', $value = '')->placeholder($placeholder = 'body text')->class('text-black p-3') }}
